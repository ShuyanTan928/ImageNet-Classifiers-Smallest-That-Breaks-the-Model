...
from .eval_min_eps import AttackResult, estimate_min_eps, _attack_success
from .utils import (
    build_transform,
    create_dataloader,
    ensure_dir,
...
def generate_plots(
    ...
    eps_grids = {
        "linf": np.array([i / 255 for i in range(9)]),
        "l2": np.linspace(0, 3.0, 11),
    }

    plot_configs = {
        "untargeted_linf": ("Untargeted Attack ($L_\\infty$)", "linf", "untargeted"),
        "untargeted_l2": ("Untargeted Attack ($L_2$)", "l2", "untargeted"),
        "targeted_linf": ("Targeted Attack ($L_\\infty$)", "linf", "targeted"),
        "targeted_l2": ("Targeted Attack ($L_2$)", "l2", "targeted"),
    }
    ...
    for plot_key, (title, norm, attack_type) in tqdm(plot_configs.items(), desc="Generating Plots"):
        plt.figure()
        for loss_name in ["ce", "cw"]:
            success_rates = []
            epsilons = eps_grids[norm]
            for eps in tqdm(epsilons, desc=f"Eps ({norm}, {loss_name}, {attack_type})", leave=False):
                num_successes = 0
                for i in range(len(all_images)):
                    image = all_images[i:i+1]
                    true_label = all_labels[i:i+1]
                    
                    target_tensor = None
                    if attack_type == "targeted":
                        target_label_value = _generate_target(true_label.item())
                        target_tensor = torch.tensor([target_label_value], device=device)

                    success, _, _ = _attack_success(
                        model, image, true_label, target_tensor,
                        eps, norm, loss_name, args.pgd_iters, mean, std
                    )
                    if success:
                        num_successes += 1
                
                success_rates.append(num_successes / len(all_images))
            
            plt.plot(epsilons, success_rates, marker='o', linestyle='-', label=f"{loss_name.upper()} Loss")
        ...
        plt.legend()
        plt.ylim(0, 1.05)
        plot_path = plot_dir / f"{plot_key}_success_rate.png"
        plt.savefig(plot_path)
        plt.close()
        print(f"Saved plot to {plot_path}")

def print_summary_tables(summaries: Dict[str, Dict]) -> None:
    print("\n" + "="*50)
    print(" " * 15 + "SUMMARY OF RESULTS")
    print("="*50)

    print("\nTable 1: Untargeted Attacks (Median ε*)")
    print("-" * 45)
    print(f"| {'Norm':<10} | {'Loss':<10} | {'Median ε*':<20} |")
    print(f"|{'-'*12}|{'-'*12}|{'-'*22}|")
    key_map = {"linf_ce_untargeted": ("L-inf", "CE"), "linf_cw_untargeted": ("L-inf", "CW"),
               "l2_ce_untargeted": ("L2", "CE"), "l2_cw_untargeted": ("L2", "CW")}
    for key, (norm, loss) in key_map.items():
        median_eps = summaries.get(key, {}).get('min_eps_median', 'N/A')
        if isinstance(median_eps, float):
            median_eps = f"{median_eps:.6f}"
        print(f"| {norm:<10} | {loss:<10} | {median_eps:<20} |")
    print("-" * 45)

    print("\nTable 2: Targeted Attacks (Median ε*)")
    print("-" * 45)
    print(f"| {'Norm':<10} | {'Loss':<10} | {'Median ε*':<20} |")
    print(f"|{'-'*12}|{'-'*12}|{'-'*22}|")
    key_map = {"linf_ce_targeted": ("L-inf", "CE"), "linf_cw_targeted": ("L-inf", "CW"),
               "l2_ce_targeted": ("L2", "CE"), "l2_cw_targeted": ("L2", "CW")}
    for key, (norm, loss) in key_map.items():
        median_eps = summaries.get(key, {}).get('min_eps_median', 'N/A')
        if isinstance(median_eps, float):
            median_eps = f"{median_eps:.6f}"
...
def main() -> None:
    ...
    sample_counter = 0
    for batch in tqdm(dataloader, desc="Finding min ε*", total=len(dataset)):
        ...
        for idx_in_batch in range(batch_size):
            ...
            for attack_type, loss_name, norm in configs:
                ...
                if attack_result.success and not example_saved:
                    example_name = f"example_{norm}_{loss_name}_{attack_type}"
                    
                    save_image(image, os.path.join(example_dir, f"{example_name}_original.png"))
                    save_image(attack_result.adv_image, os.path.join(example_dir, f"{example_name}_adversarial.png"))
                    
                    with open(os.path.join(example_dir, f"{example_name}_details.txt"), "w") as f:
                        f.write(f"Attack Example Details:\n")
                        f.write(f"  - Norm: {norm}\n")
                        f.write(f"  - Loss Function: {loss_name}\n")
                        f.write(f"  - Attack Type: {attack_type}\n")
                        f.write(f"  - True Label: {int(true_label.item())}\n")
                        f.write(f"  - Misclassified Label: {attack_result.adv_pred}\n")
                        f.write(f"  - Epsilon*: {attack_result.min_eps}\n")

                    print(f"\nSaved attack example to {example_dir}")
                    example_saved = True
    ...
    generate_plots(model, dataloader, args.output_dir, device, args)
    print_summary_tables(all_summaries)
