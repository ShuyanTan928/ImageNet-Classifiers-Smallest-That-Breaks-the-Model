...
def _attack_success(
    ...
    if eps <= 0:
        ...
        if y_target is None:
            return (pred != y_true).item(), pred.item()
        return (pred == y_target).item(), pred.item()

    alpha = eps / 4.0
    alpha = max(alpha, eps / max(pgd_iters, 1))
    ...
    for _ in range(pgd_iters):
        ...
        if loss_name == "ce":
            ...
            margin = _cw_margin(logits, y_target if y_target is not None else y_true, y_target is not None)
            loss = margin.mean()
        else:
...
def estimate_min_eps(
    ...
    *,
    ...
    def run_attack(epsilon: float) -> Tuple[bool, int]:
    ...
    success, adv_pred = run_attack(eps_upper)
    if not success:
        return AttackResult(False, None, None, attack_calls)
    ...
    for _ in range(binary_search_steps):
        mid = (lo + hi) / 2.0
        success, adv_pred = run_attack(mid)
        if success:
            best_eps = mid
            best_pred = adv_pred
            hi = mid
            ...
            lo = mid
    return AttackResult(True, best_eps, best_pred, attack_calls)
